jQuery(function($){
    var team = {
        ...
        "html": "",
        "template": "<div class=\"col-lg-3 col-md-3 col-sm-4\"> <div class=\"single_team wow fadeInUp\"> <div class=\"team_img\"> <img src=\"img/team/%s.jpg\"> </div> <h5>%s</h5> <span>%s</span> <div class=\"description\">%s</div> <div class=\"team_social\">%s</div> </div> </div>",
        ...
        "prepare": function () {
            for (var i = 0; i < this.list.length; ++i) {
                var social_list = "";
                Object.keys(this.list[i]['social']).forEach(function (site) {
                    if (site == "wechat") {
                        social_list += sprintf(team.wechat, this.site);
                    } else {
                        social_list += sprintf(team.social, this.site, site);
                    }
                });
                this.html += sprintf(this.template,
                    this.list[i]["name"].replace(/ /g,'').toLowerCase(),
                    this.list[i]["name"],
                    this.list[i]["program"],
